...
# TODO: il disaggregated clairvoyant è una merda, viene basso e non capiamo perchè
best_disaggr_revenue = clairvoyant.disaggr_revenue_given_arms(arms=[1, 3, 1, 1, 1], env=env)
...
ax[0].plot(context_generator.average_rewards, color='green', label=mode)
ax[0].axhline(y=best_revenue, color='red', linestyle='--', label='Clairvoyant (Aggregate)')
ax[0].axhline(y=best_disaggr_revenue, color='blue', linestyle='--', label='Clairvoyant (Disaggregate)')
ax[0].set_title('Average reward')
ax[1].plot(np.cumsum(context_generator.average_rewards), color='green', label=mode)
ax[1].plot(np.cumsum(best_revenue_array), color='red', linestyle='--', label='Clairvoyant (Aggregate)')
ax[1].plot(np.cumsum(best_disaggr_revenue_array), color='red', linestyle='--', label='Clairvoyant (Disaggregate)')
